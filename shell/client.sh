...
# 启动 10 个客户端，每个绑定一个不同的本地 IP
#for i in $(seq 1 5); do
#    ip="127.0.0.$i"
#    log "Launching pingpong_client with IP $ip"
#    # 后台运行，每个客户端日志单独输出
#    ./pingpong_client -p 9000 -t 1 -d 600 -c 1000 -H "$ip" > "pingpong_client_$i.log" 2>&1 &
#    sleep 10
#done
...
for i in $(seq 1 5); do
    ...
    for batch in $(seq 1 8); do
        ...
        ./pingpong_client -p 9000 -t 1 -d 600 -c 2000 -H "$ip" >> "pingpong_client_$i.log" 2>&1 &

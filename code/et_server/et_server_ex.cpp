...
public:
    ...
    DualBufferPool &buffer_pool;

    Connection(DualBufferPool &pool) : buffer_pool(pool) {}
    ...
    bool out_push(const OutEntry &e) {
        if (!out_buffer) {
            out_buffer = buffer_pool.acquire(LARGE_BLOCK);     //分配发送缓冲区
            if (!out_buffer){
                assert(0);
                return false;
            }
        ...
        out_offset += e.len;
        out_len += e.len;
        buffer_pool.retain(out_buffer);    //add发送缓冲区计数，可以避免再次分配
        return true;
    }
    ...
    void out_advance(size_t bytes_written) {
        if (bytes_written >= out_len) {
            out_len = 0;
            out_offset = 0;
            if (out_buffer) {
                buffer_pool.release_ref(out_buffer);            //回收发送缓冲区
...
public:
    ConnectionPool(size_t max_conn, DualBufferPool &dp, RingBufferPool &rp)
        ...
        for (size_t i = 0; i < max_conn; ++i) {
            storage_[i] = std::make_unique<Connection>(dp_);
        }
...
public:
    ...
    bool enqueue(Connection* c) { return q_.enqueue(c); }
    bool dequeue(Connection* c) { return q_.dequeue(c); }

private:
    MPMCRing<Connection*> q_;
};

static inline void
business_worker_echo(Connection *conn, BufferBlock *stolen_block, size_t body_len, TaskQueue &reactor_queue) {
    //logger_->debug("business_worker_echo");
    OutEntry entry{stolen_block, 0, body_len};

    conn->out_push(entry);
    while (!reactor_queue.enqueue(conn)){
        std::this_thread::yield();
    }
}
...
private:
    ...
    void on_readable(Connection *conn, std::unordered_map<int, Connection*> &conns) {
        ...
        while (true) {
            ...
            // steal body 拷贝到新块
            BufferBlock *stolen = conn->rx->steal_body_after(5, hdr.body_len);
            if (!stolen) {
                g_metrics.drops.fetch_add(1, std::memory_order_relaxed);
                break;
            }
            ...
            //logger_->debug(msg);
            business_worker_echo(conn, stolen, 5 + hdr.body_len, taskq_);
        }

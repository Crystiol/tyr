...
public:
    ...
    void run() {
        ...
        while (!global_stop_.load()) {
            ...
            //print_time(); fprintf(stderr,"loop_start\n");
            logger_->debug("loop_start");

            //print_time(); fprintf(stderr,"process_incoming_tasks start\n");
            logger_->debug("process_incoming_tasks start");
            process_incoming_tasks(conns);
            //print_time(); fprintf(stderr,"process_incoming_tasks end\n");
            logger_->debug("process_incoming_tasks end");

            //print_time(); fprintf(stderr,"epoll_wait start\n");
            logger_->debug("epoll_wait start");
            int n = epoll_wait(epfd_, evs.data(), (int)evs.size(), 1000);
            //print_time(); fprintf(stderr,"epoll_wait end\n");
            ...
            for (int i = 0; i < n; ++i) {
                ...
                if (fd == listen_fd_) {
                    //print_time(); fprintf(stderr,"accept_loop start\n");
                    logger_->debug("accept_loop start");
                    accept_loop(conns);
                    //print_time(); fprintf(stderr,"accept_loop end\n");
                    logger_->debug("accept_loop end");
                } /*else if (fd == tfd) {
                    print_time(); fprintf(stderr,"timer start\n");
                    uint64_t exp;
                    (void)read(tfd, &exp, sizeof(exp));
                    //wheel_.tick(tnow, [&](int xfd) { on_timeout(xfd, conns); });
                    print_time(); fprintf(stderr,"timer end\n");
                } */else if (fd == metrics_fd_) {
                    //print_time(); fprintf(stderr,"serve_metrics start\n");
                    logger_->debug("serve_metrics start");
                    serve_metrics(fd);
                    //print_time(); fprintf(stderr,"serve_metrics end\n");
            ...
            //print_time(); fprintf(stderr,"loop_end: %llu\n", elapse);
    ...
    void process_incoming_tasks(std::unordered_map<int, Connection *> &conns) {
        ...
        while (taskq_.dequeue(t)) {
            ...
            if (!conn->out_push(t.entry, pool_)) {
                ...
                ev.data.fd = conn->fd;
                if(epoll_ctl(epfd_, EPOLL_CTL_MOD, conn->fd, &ev))
                {
                    char* str = strerror(errno);
                    fprintf(stderr, str);
                    assert(0);

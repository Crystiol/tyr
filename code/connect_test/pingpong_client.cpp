...
static void on_connect(hio_t* io) {
    ...
    uint64_t val = seq.fetch_add(1, std::memory_order_relaxed);
    char tmp[32] = {0};
    ...
    memcpy(buf+1, &flag, sizeof(flag));
    memcpy(buf+5, &len, sizeof(len));
    memcpy(buf+9, tmp, len);

    hio_write(io, buf, 9+len);
...
static void start_connect(hloop_t* loop) {
    ...
    hio_set_unpack(io, &grpc_unpack_setting);

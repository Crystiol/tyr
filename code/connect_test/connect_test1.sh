...
ip addr add 192.168.217.129/24 dev ens32
ip addr add 192.168.217.130/24 dev ens32
ip addr add 192.168.217.131/24 dev ens32
ip addr add 192.168.217.132/24 dev ens32
ip addr show dev ens32

log "Starting multiple pingpong_client instances..."

for i in $(seq 128 132); do
    ip="192.168.217.$i"
    ...
    for batch in $(seq 1 8); do
        ...
        ./cmake-build-debug/connect_test "$ip" 192.168.217.1 5999 2000 &
